    ...
    def post(self, request, format=None):
        ...
        if code is not None:
            ...
            if len(room_result) > 0:
                ...
                print('Session key in join:', self.request.session.session_key)
                print('Room code set in session:', self.request.session['room_code'])
...
#                 room = room_result[0]
#                 self.request.session['room_code'] = code
#                 print(f"Room code {code} set in session {session_key}")
#                 return Response({'message': 'Room Joined!'}, status=status.HTTP_200_OK)
...
class LeaveRoom(APIView):
    def post(self, request, format=None):
        if 'room_code' in self.request.session:
            ...
            room_results = Room.objects.filter(host=host_id)  # Corrected here
